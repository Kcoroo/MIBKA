...
% 与原算法仅 多了一个 用于选择变异策略的 index 参数
function [Best_Fitness_BKA,Best_Pos_BKA,Convergence_curve]=mutation_BKA(pop,T,lb,ub,dim,fobj,index)
...
%% -------------------Migration behavior-------------------%
        ...
        % 与原算法仅 多了 下面一行
        XPosNew(i,:) =mutations(XPos(i,:),XPos,XLeader_Pos,lb,ub,dim,t,T,index); 
        %---------------------------------------变异策略---------------------------------------
        %----------------------------------------------------------------------------------------
        XPosNew(i,:) = max(XPosNew(i,:),lb);XPosNew(i,:) = min(XPosNew(i,:),ub); %%Boundary checking
%% --------------  Select the optimal fitness value---------%
